...
if plot:
    ...
    if not os.path.exists('./weight_result/spatial_learning_cifar10'):
        os.makedirs('./weight_result/spatial_learning_cifar10')
...
for i, datum in enumerate(dataloader):
    ...
    for b in range(batch_size):
        ...
        if plot and current_samples % 250 == 0:  # 每250个样本绘制和保存权重
            ...
            # 保存图像
            plt.savefig(f'./weight_result/spatial_learning_cifar10/weights_sample_{current_samples}.png')
            plt.close()
            
            # 保存权重为CSV文件
            # 获取权重
            weight_tensor = network.connections[("X", "E")].w
            # 转换为numpy数组
            weight_array = weight_tensor.cpu().numpy()
            # 创建DataFrame
            weight_df = pd.DataFrame(weight_array)
            # 保存为CSV
            weight_df.to_csv(f'./weight_result/spatial_learning_cifar10/weights_sample_{current_samples}.csv', index=False)
        ...
        if current_samples % update_interval == 0 and current_samples > 0:
            ...
            # 在每个update_interval结束后，只保存权重，不绘制其他内容
            if plot:
                # 获取权重并生成正方形权重图
                input_exc_weights = network.connections[("X", "E")].w
                square_weights = get_square_weights(input_exc_weights.view(32*32, n_neurons), n_sqrt, 32)
                ...
                plt.figure(figsize=(10, 10))
                weights_im = plot_weights(square_weights, im=None)
                plt.title(f"训练权重 - 样本 {current_samples} (update_interval)")
                ...
                # 保存图像
                plt.savefig(f'./weight_result/spatial_learning_cifar10/weights_update_{current_samples}.png')
                plt.close()
                ...
                weight_df = pd.DataFrame(weight_array)
                # 保存为CSV
                weight_df.to_csv(f'./weight_result/spatial_learning_cifar10/weights_update_{current_samples}.csv', index=False)
...
if plot:
    ...
    # 保存最终权重图像
    plt.savefig('./weight_result/spatial_learning_cifar10/final_weights.png')
    plt.close()
    ...
    weight_df = pd.DataFrame(weight_array)
    # 保存为CSV
    weight_df.to_csv('./weight_result/spatial_learning_cifar10/final_weights.csv', index=False)
    print("权重已保存为CSV文件")
