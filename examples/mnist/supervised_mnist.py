...
for i, datum in enumerate(dataloader):
    ...
    if plot:
        ...
        inpt_axes, inpt_ims = plot_input(#图1
            image.sum(1).view(28, 28), inpt, label=label, axes=inpt_axes, ims=inpt_ims
        )
        spike_ims, spike_axes = plot_spikes(#图2
            {layer: spikes[layer].get("s").view(time, 1, -1) for layer in spikes},
            ims=spike_ims,
            axes=spike_axes,
        )
        weights_im = plot_weights(square_weights, im=weights_im)#图3    
        assigns_im = plot_assignments(square_assignments, im=assigns_im)#图4
        perf_ax = plot_performance(accuracy, x_scale=update_interval, ax=perf_ax)#图5
        voltage_ims, voltage_axes = plot_voltages(
            voltages, ims=voltage_ims, axes=voltage_axes#图6
        )
