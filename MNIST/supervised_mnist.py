    ...
    for b in range(batch_size):
        ...
        if plot:
            ...
            if plot and current_samples % 250 == 0:
                plt.savefig(f'./weight_result/STDP_mnist/weights_sample_{current_samples}.png')   
            plt.pause(1e-8)
...
if plot:
    ...
    # 确保目录存在
    if not os.path.exists('./weight_result/STDP_mnist'):
        os.makedirs('./weight_result/STDP_mnist')
        
    # 保存最终权重图像
    plt.savefig('./weight_result/STDP_mnist/final_weights.png')
    plt.close()
